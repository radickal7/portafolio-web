...
export default function Projects() {
  ...
  useEffect(() => {
    document.title = `${t("nav.projects")} | JuanDev — ${t("projectsPage.heroTitle")}`
    const m = document.querySelector('meta[name="description"]')
    if (m) m.setAttribute("content", t("projectsPage.heroSubtitle"))
  }, [t])
    ...
    <main className="pt-[72px] pb-16">
      ...
      <section className="relative overflow-hidden border-b border-white/[0.06]">
        ...
        <div className="max-w-[1280px] mx-auto px-5 lg:px-6 py-10 md:py-14">
          <h1 className="text-[32px] md:text-[40px] font-extrabold tracking-[-0.03em] text-white leading-[0.95]">{t("projectsPage.heroTitle")}</h1>
          <p className="mt-4 text-slate-400 max-w-2xl">{t("projectsPage.heroSubtitle")}</p>
        </div>
      </section>

      {/* Filters */}
      <div className="max-w-[1280px] mx-auto px-5 lg:px-6 py-6">
        <div className="flex flex-wrap gap-2" role="tablist" aria-label={t("projectsPage.filtersLabel")}>
          {categories.map((c) => (
        ...
        <div className="mt-2 text-xs text-slate-500">{filtered.length} — {active}</div>
      </div>
            ...
            <article key={p.slug} className="group rounded-[20px] border border-white/10 bg-gradient-to-b from-white/[0.06] to-white/[0.02] overflow-hidden flex flex-col hover:border-white/15 transition">
              <div className="aspect-[16/10] overflow-hidden bg-[#0b1220] relative">
                ...
                <div className="absolute top-3 left-3 flex gap-2">
                  ...
                  {p.featured && <span className="text-[11px] font-semibold px-2.5 py-1 rounded-full bg-indigo-500 text-white">{t("projectsPage.featured")}</span>}
                </div>
              ...
              <div className="p-5 flex flex-col flex-1">
                ...
                <div className="mt-4 flex gap-2">
                  <Link to={`/projects/${p.slug}`} className="flex-1 bg-white text-slate-900 rounded-full py-2.5 text-sm font-semibold text-center hover:bg-slate-100">{t("projectsPage.viewCase")} →</Link>
                  {p.liveUrl ? <a href={p.liveUrl} target="_blank" rel="noreferrer" className="px-4 py-2.5 rounded-full border border-white/15 text-white text-sm font-semibold hover:bg-white/5">{t("projectsPage.viewProject")}</a> : null}
                </div>
              </div>
            </article>
          ))}
        </div>
        {filtered.length === 0 && <div className="text-center py-16 text-slate-400">{t("projectsPage.noResults")}</div>}
      </div>
    </main>
  )
}
